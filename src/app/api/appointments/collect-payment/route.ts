...
export async function POST(req: Request) {
  try {
    ...
    // In-office trials don't generate a receipt — no TR-XXXXXX is consumed and no PDF is rendered.
    ...
    const receiptLabel = isInOfficeTrial
      ? 'In-office trial recorded'
      : receiptType === 'trial'
    ...
    let htmlTemplateIdUsed: string | undefined;
    if (!isInOfficeTrial) {
      ...
      htmlTemplateIdUsed = result.templateId;
    }

    if (
      !isInOfficeTrial &&
      htmlTemplateIdFromClient &&
      htmlTemplateIdUsed &&
      htmlTemplateIdFromClient !== htmlTemplateIdUsed
    ) {
      console.warn(
    ...
    if (notify.length === 0) {
      ...
      try {
        const subjectPrefix = isInOfficeTrial ? '[Staff in-office trial]' : '[Staff payment]';
        const htmlAttachmentLine = isInOfficeTrial
          ? '<p>No PDF attached — in-office trials do not generate a receipt. The trial visit has been recorded on the enquiry.</p>'
          : '<p>PDF attached — same template family as CRM (Invoice Manager). Verify in CRM before sending to the patient if required.</p>';
        const textFooter = isInOfficeTrial
          ? `In-office trial recorded — no receipt is generated for in-office trials.`
          : `Verify in CRM and send the official document to the patient if approved.`;
        await sendStaffPaymentNotifyEmail({
          to: notify,
          subject: `${subjectPrefix} ${receiptLabel} — ${patientName} (₹${amount}) · Generated by ${staffName}`,
          text: [
            isInOfficeTrial
              ? `A staff member recorded an in-office trial.`
            ...
            `Mode: ${paymentMethod}`,
            `Type: ${isInOfficeTrial ? 'trial (in-office)' : receiptType}`,
...
<p>${isInOfficeTrial ? 'A staff member recorded an in-office trial.' : 'A staff member submitted a payment request.'}</p>
<ul>
<li><b>Patient:</b> ${escapeHtml(patientName)}</li>
<li><b>Amount:</b> ₹${amount.toFixed(2)}</li>
<li><b>Mode:</b> ${escapeHtml(paymentMethod)}</li>
<li><b>Type:</b> ${escapeHtml(isInOfficeTrial ? 'trial (in-office)' : receiptType)}</li>
