...
export async function exportToExcel(summary: ProfitSummary, dateLabel: string): Promise<void> {
  ...
  const summaryData = [
    ...
    ['  Cash expenses (register, Expenses category)', summary.totalCashOutflows],
    ['  Managed expenses (Expenses module)', summary.totalManagedExpenses],
...
export async function exportToPdf(summary: ProfitSummary, dateLabel: string): Promise<void> {
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['Cash expenses (register)', formatCurrency(summary.totalCashOutflows)],
      ['Managed expenses (Expenses module)', formatCurrency(summary.totalManagedExpenses)],
  ...
  if (summary.centerRows?.length) {
    ...
    autoTable(doc, {
      ...
      body: summary.centerRows.map((r) => [
        ...
        formatCurrency(r.managedExpenses),
